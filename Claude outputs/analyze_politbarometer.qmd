...
pb |>
  filter(!is.na(spd)) |>
  group_by(year) |>
  summarise(mean_spd = weighted.mean(spd, weight, na.rm = TRUE)) |>
...
m0_spd <- brm(spd | weights(weight) ~ worker,
              data = pb_spd,
              family = bernoulli(),
...
m1_spd <- brm(spd | weights(weight) ~ worker + (1 | year),
              data = pb_spd, family = bernoulli(),
...
m2a_spd <- brm(spd | weights(weight) ~ worker + (1 + worker | year),
               data = pb_spd, family = bernoulli(),
               prior = priors_loyalty_spd,
               chains = 4, cores = 4, seed = 1,
               backend = "cmdstanr",
...
icv_by_year_spd |>
  ...
  labs(
    ...
    title = "Class voting gap in SPD support, by survey year",
    caption = "Posterior median, 66% and 95% credible intervals"
  ) +
  theme_minimal(base_size = 12) +
  theme(
...
pb_weighted_contrib_spd <- pb_spd |>
  filter(!is.na(worker), !is.na(spd)) |>
  group_by(year, spd) |>
  summarise(
    p_worker = weighted.mean(worker, weight, na.rm = TRUE),
...
m_contrib_spd <- brm(
  worker | weights(weight) ~ spd + (1 + spd | year),
  data = pb_spd, family = bernoulli(link = "logit"),
  prior = priors_contribution_spd,
  backend = "cmdstanr",
  threads = threading(4),
  chains = 4, cores = 4, seed = 1,
...
contribution_by_year_spd <- pb_spd |>
  distinct(year) |>
